...
get_sink_volume_and_mute_status() {
    ...
    v=$(pactl list sinks | grep -A 10 "Sink #$sink_id" | grep 'Volume:' | awk '{print $5}' | cut -d'%' -f1)
    # Extract mute status
    mute_status=$(pactl list sinks | grep -A 15 "Sink #$sink_id" | grep 'Mute:' | awk '{print $2}')
    m=$([[ "$mute_status" == "yes" ]] && echo "true" || echo "false")
    ...
    # Format the output
    jq -nc --arg v $v --arg m $m '.volume = $v | .muted = $m'
}
